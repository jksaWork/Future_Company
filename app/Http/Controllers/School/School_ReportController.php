...
class School_ReportController extends Controller
{
    ...
    public function report_spending(Request $request)
    {
        ...
        if ($b == null or $e == null) {
            ...
            return view('admin.School_teachers.School_Repoet.index', compact('spendings'));
        } else {
            $spendings = school_spendings::whereBetween('created_at', [$b, Carbon::parse($e)->endOfDay(),])->get();
            return view('admin.School_teachers.School_Repoet.index', compact('spendings'));
        }
    ...
    public function report_employee(Request $request)
    {
        ...
        if ($b == null or $e == null) {
            ...
            $employee = school_teachers::where('status', 1)->get();
            $allowancesS = School_Teachers_allowances::where('status', 1)->get();
            // return  $employee;
            return view('admin.School_teachers.School_Repoet.employee', compact('employee', 'allowancesS'));
        } else {
            // dd('ok');
            $employee = school_teachers::where('status', 1)->whereBetween('created_at', [$b, Carbon::parse($e)->endOfDay(),])->get();
            $allowancesS = School_Teachers_allowances::where('status', 1)->get();
            return view('admin.School_teachers.School_Repoet.employee', compact('employee', 'allowancesS'));
            return  $employee;
        }
    ...
    public function report_employee_allowances(Request $request)
    {
        ...
        if ($b == null or $e == null) {
            ...
            $employee_allowances = School_Teachers_allowances::where('status', 0)->get();
            return view('admin.School_teachers.School_Repoet.employee_allowances', compact('employee_allowances'));
        } else {
            $employee_allowances = School_Teachers_allowances::where('status', 0)->whereBetween('created_at', [$b, Carbon::parse($e)->endOfDay(),])->get();
